...
| **Frequência de Controle** | **Frequências de 50Hz a 100Hz** no microcontrolador para motores de alto torque. | **Frequência de 50Hz (Refatorada)**: Loop PI rodando a cada 20ms com Feedforward. | **Estabilidade Física**. A nova malha PI a 50Hz corrigiu oscilações de atraso de comando que faziam o robô guinar. |
